...
4. Se pide que refactorices el proyecto en su estado actual añadiendo una nueva clase `Tarea` y modificando la clase `GestorTareas` para que siga pasando los tests actuales. Ten en cuenta que:
  ...
  * Siguiendo el principio de _responsability driven-design_, los objetos de la clase `Tareas` deben disponer de un método que permita obtener su representación en formato `String`. Este método se debe denominar `toString`.
