...
4. Implementa la segunda clase de forma que (realiza un commit por cada punto). __Es obligatorio__ que implementes una clase de test en la que que haya un método por cada commit que verifique el correcto funcionamiento de dicho commit. 
    ...
    * **Commit 10**: Permita eliminar los objetos cuyo atributo cumpla una determinada condición. Por ejemplo, en nuestro caso se podría indicar que queremos eliminar todos los coches que tienen como año de fabricación un año anterior a uno que indique el usuario.
